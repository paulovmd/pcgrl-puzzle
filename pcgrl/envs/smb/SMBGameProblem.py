...
class SMBGameProblem(GameProblem):    
    ...
    def __init__(self, rows = 0, cols = 0, border = False):
        ...
        super(SMBGameProblem, self).__init__(w = self.width, h = self.height, tile_w = self.tile_size, tile_h = self.tile_size)            
    ...
    def change_tile(self, x, y, val):
        if not self.blocked:
            ...
            if val == Background.ID:
                tile  = Background(id =Background.ID, x = col * state_w, y = row * state_h)                    
                ...
                tile  = DoorEntrance(id = DoorEntrance.ID, x = col * state_w, y = row * state_h)                    
                self.addBases(tile)
            elif val == DoorExit.ID:
                tile  = DoorExit(id = DoorExit.ID, x = col * state_w, y = row * state_h)                    
                self.addBases(tile)
            elif val == Ladder.ID:
                tile  = Ladder(id = Ladder.ID, x = col * state_w, y = row * state_h)                    
                self.addBases(tile)                                                
            elif val == Coin.ID:
                tile  = Coin(id = Coin.ID, x = col * state_w, y = row * state_h)                    
                self.addLevelObjects(tile)                        
            elif val == Key.ID:
                tile  = Key(id = Key.ID, x = col * state_w, y = row * state_h)                    
                self.addLevelObjects(tile)                        
            elif val == Player.ID:
                tile  = Player(id = Player.ID, x = col * state_w, y = row * state_h)                    
                self.addPlayers(tile)
            elif val == Enemy.ID:
                tile  = Enemy(id = Enemy.ID, x = col * state_w, y = row * state_h)                    
                self.addPlayers(tile)            
    ...
    def get_map_stats(self):
        map_stats = {
            ...
            "Background"      : self.calc_tiles(self.map, self.dic_tiles,  "Background"),
            ...
            "regions_ground"  : self.calc_regions(self.map, Background.ID, [Background.ID, Player.ID, CoinGold.ID, Ladder.ID, Enemy.ID, Key.ID])
        }
    ...
    def is_done(self, stats):
        ...
        done_resources = regions_ground == 1 and \
        door_exit == 1 and door_entrance == 1 and key == 1 and player == 1 and \
        self.in_range(coins, self._range_coins["min"], self._range_coins["max"]) and \
        self.in_range(enemy, self._range_enemies["min"], self._range_enemies["max"])
